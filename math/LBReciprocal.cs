...
{
    public class LBReciprocal : LogicBlock
    {
        public class Info : LogicBlockInfo
        {
            public string UID => "";

            public string Title => "Reciprocal 1/x";
            ...
            public string description => "Reciprocal of port IN value";

            public string instrumentsMenuCategoryTitle => "Math";
        }

        LogicBlockPort inputPortA,outputPort;

        public LBReciprocal(IErrorManager _errorManager, IPluginsManager _pluginsManager, IEventsManager _eventsManager, IWindowsManager _windowsManager, InstrumentDefinition instrumentDefinition) : base(_errorManager, _pluginsManager, _eventsManager, _windowsManager, instrumentDefinition)
        {   
        ...
        {
            if (inputPortA.getData(false) != null)
            {
                outputPort.setData(Math.ReciprocalEstimate(inputPortA.getData(false)), false, false);
